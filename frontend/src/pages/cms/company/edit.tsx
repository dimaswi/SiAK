...
const pageOptions = [
  ...
  },
  {
    ...
    items: [
      ...
      { value: "home-highlight-4", label: "Poin Sorotan 4 (Tambahan)" },
    ]
  },
  {
...
export default function CMSCompanyEdit() {
  const { id } = useParams()
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await dialog.alert("Gambar berhasil diupload")
    } catch (err: any) {
  ...
  const submit = async () => {
    try {
      ...
      await dialog.alert("Halaman profile berhasil diperbarui")
      navigate("/cms/company")
    } catch (err: any) {
      await dialog.alert(err.response?.data?.message || "Gagal update halaman")
    }
  }
  ...
  return (
    <PageShell
      ...
      description="Perbarui struktur company profile dan pengaturan SEO halaman portal publik."
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_380px]">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="bg-slate-50 border-b border-slate-200 px-4 py-3">
            ...
            <p className="text-xs text-slate-400 mt-0.5">Perbarui isi halaman company profile yang akan tampil di portal publik.</p>
          </div>
