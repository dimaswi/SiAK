...
const initialForm = {
  nip: "", password: "", nuptk: "", full_name: "", gender: "L",
  birth_place: "", birth_date: "", religion: "", nationality: "WNI",
  marital_status: "", nik: "", npwp: "",
  employee_status: "", position: "", subject: "", teaching_hours: 0,
  rank: "", sk_number: "", join_date: "",
  education_level: "", education_major: "", university: "",
  graduation_year: 0, cert_number: "",
  phone: "", email: "",
  address: "", rt_rw: "", village: "", district: "",
  city: "", province: "", postal_code: "",
  ptk_type: "", sk_appointment: "", sk_date: "", appointment_institution: "",
  salary_source: "", bank_name: "", bank_account_no: "", bank_account_name: "", spouse_name: ""
}
...
export default function TeacherCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If we have a photo and got an ID back, upload photo
      // The create endpoint doesn't return ID directly, so we fetch by NIP
      if (photoFile && res.status === 201) {
        // Fetch to get the ID
        try {
          const listRes = await axios.get(`${API}/teachers?search=${formData.nip}&limit=1`)
          const teacher = listRes.data?.data?.[0]
          if (teacher?.id) {
            const fd = new FormData()
            fd.append("photo", photoFile)
            await axios.post(`${API}/upload/photo?entity_type=teacher&entity_id=${teacher.id}`, fd)
          }
            ...
            <div className="flex-1 pb-24">
              <TabsContent value="profil" className="m-0 focus-visible:outline-none flex flex-col gap-6">
                ...
                <FormSection title="Akun Login" description="NIP digunakan sebagai username login guru.">
                  ...
                  <Field label="Password" required><Input name="password" type="password" required value={formData.password} onChange={handleChange} /></Field>
                </FormSection>
